...
/**
 * The sandbox's reference one-handed weapon.
 *
 * Everything here is data. Timings that must agree with an authored clip are
 * derived from the generated animation manifest rather than retyped, so a
 * pipeline reskin cannot silently desynchronise gameplay from the motion.
 */
...
export const STRAIGHT_SWORD = defineWeapon({
  id: "straight-sword",
  ...
  stats: {
    ...
    guard: {
      // Mid-band for a weapon guard (see WEAPON_STABILITY_BAND). Deliberately
      // unchanged from the value this sandbox was balanced against, so adding
      // the stat did not retune blocking; shields will land above 0.64.
      stability: 0.58,
      absorption: { physical: 0.92 },
    },
  },
  visual: {
    asset: "weapon-steel-sword.glb",
    // Identity: the rig's own weapon-socket convention is applied once from the
    // character manifest, and a plain sword has no offset of its own.
    held: {
      socket: "Weapon",
      localPosition: [0, 0, 0],
