...
describe("resolveHit", () => {
  it("ignores contact during dodge invulnerability", () => {
    const result = resolveHit(100, 100, { attack: light, guarding: false, iframe: true, execution: null });
    expect(result.kind).toBe("iframe");
  });

  it("still lands an execution through invulnerability", () => {
    const result = resolveHit(100, 100, { attack: riposte, guarding: false, iframe: true, execution: "riposte" });
    expect(result.kind).toBe("execution");
  });

  it("subtracts weapon damage on a clean hit and flags heavy attacks", () => {
    const result = resolveHit(100, 100, { attack: heavy, guarding: false, iframe: false, execution: null });
  ...
  it("reports a kill when damage empties health", () => {
    const result = resolveHit(light.damage - 1, 100, { attack: light, guarding: false, iframe: false, execution: null });
    expect(result.kind).toBe("hit");
    if (result.kind !== "hit") return;
  ...
  it("blocks with a full stamina bar", () => {
    const result = resolveHit(100, 100, { attack: light, guarding: true, iframe: false, execution: null });
  ...
  it("breaks a guard with an empty stamina bar and applies guard-break damage", () => {
    const result = resolveHit(100, 0, { attack: light, guarding: true, iframe: false, execution: null, guardBreakDamage: 18 });
  ...
  it("bypasses guard for executions", () => {
    const result = resolveHit(100, 100, { attack: riposte, guarding: true, iframe: false, execution: "backstab" });
    expect(result.kind).toBe("execution");
  });
});
