...
/**
 * Bind every generated run/edge artifact to a stable qualitative-review id.
 * The artifact path is added only after FFmpeg has emitted the real image, so
 * a checklist entry can never masquerade as evidence that was not rendered.
 */
export function buildRunReviewChecklist(strips, transitionBoundaryStrips) {
  const checklist = [];
  const seenIds = new Set();
  const seenArtifacts = new Set();
  const append = (item, kind) => {
    if (!item || typeof item.reviewId !== "string" || item.reviewId.length === 0) {
      throw new TypeError(`${kind} artifact needs a non-empty reviewId`);
    }
    if (typeof item.path !== "string" || item.path.length === 0) {
      throw new TypeError(`${item.reviewId} needs a non-empty artifact path`);
    }
    if (seenIds.has(item.reviewId)) throw new TypeError(`duplicate review checklist id ${item.reviewId}`);
    if (seenArtifacts.has(item.path)) throw new TypeError(`duplicate review artifact path ${item.path}`);
    seenIds.add(item.reviewId);
    seenArtifacts.add(item.path);
    checklist.push({
      id: item.reviewId,
      kind,
      artifact: item.path,
      ...
      ...(kind === "animation-run"
        ? {
            ...
            commandSerial: item.commandSerial ?? null,
            part: item.part,
            parts: item.parts,
          }
        : {
            fromAnimation: item.fromAnimation,
            fromOccurrence: item.fromOccurrence,
            fromCommandSerial: item.fromCommandSerial ?? null,
            toAnimation: item.toAnimation,
            toOccurrence: item.toOccurrence,
            toCommandSerial: item.toCommandSerial ?? null,
            transitionTime: item.transitionTime,
          }),
    });
  };
  for (const strip of strips ?? []) append(strip, "animation-run");
  for (const strip of transitionBoundaryStrips ?? []) append(strip, "animation-transition");
  return checklist;
}

function contractRuns(contract, actor) {
  const requirements = contract?.requiredAnimationRuns?.[actor] ?? [];
  const occurrences = new Map();
  return requirements.map((requirement, index) => {
    const { state } = normalizeRequirement(requirement, index);
    const occurrence = (occurrences.get(state) ?? 0) + 1;
    occurrences.set(state, occurrence);
    return { actor, index, state, occurrence };
  });
}

/** Return every statically required adjacent edge, annotated with its scene. */
export function collectContractEdges(contracts) {
  const edges = [];
  for (const [scenario, contract] of Object.entries(contracts ?? {})) {
    for (const actor of ["player", "enemy"]) {
      for (const edge of buildRunEdges(contractRuns(contract, actor))) {
        edges.push({ scenario, ...edge });
      }
    }
  }
  return edges;
...
function obligationMatches(edge, obligation) {
  return edge.scenario === obligation.scenario
    && edge.actor === obligation.actor
    && edge.from === obligation.from
    && edge.to === obligation.to
    && (obligation.fromOccurrence === undefined || edge.fromOccurrence === obligation.fromOccurrence)
    && (obligation.toOccurrence === undefined || edge.toOccurrence === obligation.toOccurrence);
}

/**
 * Prove that each curated FSM transition obligation is an adjacent edge in a
 * declared scenario path. Ambiguous repeated edges must name their occurrences.
 */
export function validateTransitionObligations(contracts, obligations) {
  ...
  for (const [index, obligation] of obligations.entries()) {
    if (!obligation || typeof obligation.id !== "string" || obligation.id.length === 0) {
      throw new TypeError(`transition obligation ${index + 1} needs a non-empty id`);
    }
    if (seenIds.has(obligation.id)) throw new TypeError(`duplicate transition obligation id ${obligation.id}`);
    seenIds.add(obligation.id);
    for (const field of ["scenario", "actor", "from", "to"]) {
      if (typeof obligation[field] !== "string" || obligation[field].length === 0) {
        throw new TypeError(`${obligation.id} needs a non-empty ${field}`);
      }
    }
    const matches = edges.filter((edge) => obligationMatches(edge, obligation));
    if (matches.length === 0) {
      failures.push(`${obligation.id} is not an adjacent ${obligation.actor} ${obligation.from}->${obligation.to} edge in ${obligation.scenario}`);
    } else if (matches.length > 1) {
      failures.push(`${obligation.id} matches ${matches.length} edges; specify fromOccurrence/toOccurrence`);
    }
    coverage.push({ id: obligation.id, matches: matches.map(({ id, scenario }) => ({ id, scenario })) });
  }

  return { pass: failures.length === 0, edges, coverage, failures };
}
